...
const Classroom = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${!editable ? `<div class="${"text-center text-sm m-4"}">Alunos: ${escape(studentsPresentAmount)} | Cadeiras: ${escape(chairsAmount)} | Cadeiras vazias: ${escape(emptyChairsAmount)} | Buracos: ${escape(spacesAmount)}</div>` : ``}
<div style="${"grid-template-columns: repeat(" + escape(columnsAmount, true) + ", minmax(0, 1fr));"}" class="${"grid gap-4 grid-cols-5"}"${add_attribute("this", classroomMapAsElement, 0)}>${columns.length ? each(columns, (column, columnIndex) => {
      ...
      default: () => {
        return `${each(column, (student, studentIndex) => {
          return `${validate_component(Wrapper, "Wrapper").$$render($$result, { editable }, {}, {
            default: () => {
              return `
          ${columnIndex == 0 && editable ? `<span class="${"w-2"}">${escape(studentIndex + 1)}</span>
...
//# sourceMappingURL=DateInput-042e2584.js.map
