<script>
  ...
  import { classroomMapColumnsData } from '../data/classroomMapColumnsData.js';
  import { getStudentsPresentAmount, getChairsAmount, getSpacesAmount, getEmptyChairsAmount } from '../getStats.js';
  import Column from "./Column.svelte";

  // --------- Dados ---------
  export let data; // JSON com os dados do mapa de sala
...
<!-- Estatísticas -->
{#if !editable}
  <div class="text-center text-sm m-4">Alunos: {studentsPresentAmount} | Cadeiras: {chairsAmount} | Vazias: {emptyChairsAmount} | Buracos: {spacesAmount}</div>
{/if}
