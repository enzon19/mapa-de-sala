...
export async function load({ params, url }) {
  ...
  const requestedDate = DateTime.local(
    Number(params.year),
    Number(params.month),
    Number(params.day),
    0,
  ...
  let classroomMapData =
    (
      await supabase
        .from("classroomMap")
        .select("*")
        .eq("day", requestedDate.toISODate())
        .limit(1)
    ).data[0] || []; // day, layout, tags
  ...
  studentsData.push(
    { id: "clssrmmp_empty", name: "" },
    { id: "clssrmmp_space", name: null }
  );
  classroomMapData.layout = layout.map((column) =>
    column.map((id) => studentsData.find((student) => student.id === id))
  ); // substituir cada ID da array com o layout pelo objeto com o nome e id das pessoas
